 ...
 */
function aj_customize_register( $wp_customize ) 
{
    $wp_customize->get_setting('blogname')->transport         = 'postMessage';
    $wp_customize->get_setting('blogdescription')->transport  = 'postMessage';
    $wp_customize->get_setting('header_textcolor')->transport = 'postMessage';
}
add_action('customize_register', 'aj_customize_register');

/**
 * Customizer Sanitization Functions
 */
function aj_sanitize_text( $input ) 
{
    return wp_kses_post(force_balance_tags($input));
}

function aj_sanitize_checkbox( $input ) 
{
    if (1 == $input ) {
        return 1;
    } else {
...
/**
 * Customizer AJ Additions
 */

function aj_add_customizer_theme_options($wp_customize) 
{
    ...
    */
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            $wp_customize, 'aj_add_link_color', array(
            'label'            => __('Body Link Color', 'some-like-it-neat'),
            'section'        => 'colors',
            'settings'        => 'aj_add_link_color',
            'priority'        => 6,
            )
        ) 
    );
    ...
    */
    ...
    $wp_customize->add_setting(
        ...
        array(
        'default'            => 'Menu',
        'sanitize_callback' => 'aj_sanitize_text',
        )
    );
    $wp_customize->add_control(
        'aj_mobile_nav_label',
    ...
    // Mobile Settings
    $wp_customize->add_setting(
        'aj_mobile_hide_arrow',
        array(
        'default'        => 'No',
        'sanitize_callback'    => 'aj_sanitize_checkbox',
        )
    );
    ...
    $wp_customize->add_section(
        'aj_footer_section_settings', array(
        'priority' => 10,
        'capability' => 'edit_theme_options',
        'theme_supports' => '',
    ...
    $wp_customize->add_control(
        'aj_footer_left',
        array(
        'section'    => 'aj_footer_section_settings',
...
add_action('customize_register', 'aj_add_customizer_theme_options');

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */

function aj_customize_preview_js() 
{
    wp_enqueue_script('aj_customizer', get_stylesheet_directory_uri() . '/library/vendors/customizer/js/customizer.js', array( 'customize-preview' ), '20130508', true);
}
add_action('customize_preview_init', 'aj_customize_preview_js');
