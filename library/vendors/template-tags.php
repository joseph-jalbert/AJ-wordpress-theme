...
if ( ! function_exists( 'aj_comment' ) ) :
	/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
	function aj_comment( $comment, $args, $depth ) {
...
endif; // ends check for aj_comment()

if ( ! function_exists( 'aj_posted_on' ) ) :
	/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
	function aj_posted_on() {
...
function aj_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so aj_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so aj_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in aj_categorized_blog.
 */
function aj_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'aj_category_transient_flusher' );
add_action( 'save_post',     'aj_category_transient_flusher' );
